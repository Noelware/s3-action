...
(async () => {
  ...
  const s3 = new S3(accessKey, secretKey, bucketName, useWasabi, region, endpoint);
  ...
  for await (const file of excludePatterns.globGenerator()) shouldExclude.push(file);
  ...
  for await (const file of dirGlob.globGenerator()) {
    ...
    const format = config.getInput('object-format', '')!.replace(/[$]\(([\w\.]+)\)/g, (_, key) => {
      switch (key) {
        case 'file':
